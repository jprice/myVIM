...
function! DojoGenerateCommands(from)
  ...
  let commandsMap = {'TestPage':'DojoTestPage','TestCode':'DojoTestCode','Style':'DojoCss','Template':'DojoTemplate','Code':'DojoCode'}
  for dojoTo in dojoTos 
    exec 'command! -buffer '.commandsMap[dojoTo].' call Dojo2'.dojoTo.'(DojoFrom'.a:from.'())'
  endfor
endfunction

"global variable g:dojo_root must be set
function! DojoOpenClassUnderCursor()
	let l:fname=substitute(expand("<cfile>"), '\.', "/", "g")

	if strpart(l:fname, strlen(l:fname) - 4, 4) == 'html'
		let l:fname=substitute(l:fname, '\/html$', ".html", "")
	else
		let l:fname=g:dojo_root."/".l:fname.".js"
	endif

	execute 'find '.l:fname
endfunction
