...
syn region cssMediaBlock transparent matchgroup=cssBraces start='{' end='}' contains=cssTagName,cssError,cssComment,cssDefinition,cssURL,cssUnicodeEscape,cssIdentifier,cssFontProp
...
syn region cssFontDescriptorBlock contained transparent matchgroup=cssBraces start="{" end="}" contains=cssComment,cssError,cssUnicodeEscape,cssFontProp,cssFontAttr,cssCommonAttr,cssStringQ,cssStringQQ,cssFontDescriptorProp,cssValue.*,cssFontDescriptorFunction,cssUnicodeRange,cssFontDescriptorAttr,vars
...
syn region cssDefinition transparent matchgroup=cssBraces start='{' end='}' contains=css.*Attr,css.*Prop,cssComment,cssValue.*,cssColor,cssURL,cssImportant,cssError,cssStringQ,cssStringQQ,cssFunction,cssUnicodeEscape,vars,mixins,cssDefinition,interpolation,parentSelector,cssClassName,cssTagName,cssPseudoClass,cssIdentifier
...
syn match cssBoxProp contained "-moz-user-select"
syn match cssBoxProp contained "-khtml-user-select"

" scss
syn region mixins   start="@[a-z0-9\-_]\+" end="\s\+[a-z0-9]\+" 
syn region vars   start="\$[a-z0-9\-_]\+" end="\s*" 
syn region interpolation start="#{" end="}"
syn match parentSelector contained "&"
...
if version >= 508 || !exists("did_scss_syn_inits")
  if version < 508
    let did_scss_syn_inits = 1
    command -nargs=+ HiLink hi link <args>
  else
    command -nargs=+ HiLink hi def link <args>
  endif

  HiLink interpolation Special
  HiLink parentSelector Special
  HiLink vars Statement
  HiLink mixins Constant
